...
test("injectGuidance creates file when it does not exist", () => {
	...
	try {
		const result = injectGuidance(filePath, [{ id: "intro", content: "# Hello" }]);
...
test("injectGuidance updates existing blocks with same id", () => {
	const filePath = tmpFile();
	try {
		injectGuidance(filePath, [{ id: "intro", content: "Version 1" }]);

		const result = injectGuidance(filePath, [{ id: "intro", content: "Version 2" }]);
		assert.equal(result.modified, true);
		assert.deepEqual(result.added, ["intro"]);
...
test("injectGuidance adds new block alongside existing ones", () => {
	const filePath = tmpFile();
	try {
		injectGuidance(filePath, [{ id: "intro", content: "Hello" }]);
		injectGuidance(filePath, [{ id: "tips", content: "## Tips\n\nUse teams." }]);
...
test("injectGuidance treats empty content as removal", () => {
	...
	try {
		...
		const result = injectGuidance(filePath, [{ id: "a", content: "" }]);
