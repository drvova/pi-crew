...
test("redactJsonLine strips api_key secrets from JSON lines", () => {
	const raw = JSON.stringify({ event: "tool_call", args: { api_key: "sk-abc123456789" } });
...
test("redactJsonLine strips bearer tokens from JSON lines", () => {
	const raw = JSON.stringify({ event: "message", headers: { Authorization: "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9" } });
	const redacted = redactJsonLine(raw);
...
test("transcript on disk does not contain raw secrets after redaction", () => {
	...
	const rawLine = JSON.stringify({ type: "tool_result", output: { api_key: secretValue } });
