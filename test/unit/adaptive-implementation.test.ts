...
test("adaptive plan parser uses planner-selected phases instead of fixed fanout", () => {
	...
	assert.deepEqual(
		plan?.phases.map((phase) => phase.tasks.length),
		[2, 1, 3],
	);
	assert.deepEqual(
		plan?.phases.flatMap((phase) => phase.tasks.map((task) => task.role)),
		["explorer", "analyst", "executor", "reviewer", "test-engineer", "writer"],
	);
});
			...
			phases: [
				{
					name: "review",
					tasks: [
						{ role: "code-review", task: "Review" },
						{ role: "mystery", task: "Skip me" },
					],
				},
			],
		})}\nADAPTIVE_PLAN_JSON_END`,
...
test("implementation workflow produces runnable result with mock child-pi", async () => {
	...
	try {
		...
		assert.ok(
			["blocked", "running", "completed", "needs_attention"].includes(loaded?.manifest.status ?? ""),
...
test("implementation workflow with PI_CREW_ADAPTIVE_REPAIR=0 behaves consistently", async () => {
	...
	try {
		...
		assert.ok(
			["blocked", "running", "completed"].includes(loaded?.manifest.status ?? ""),
...
test("requirePlanApproval blocks mutating adaptive tasks until approved", async () => {
	...
	try {
		...
		const deniedApproval = await handleTeamTool(
			{
				action: "api",
		...
		const approval = await handleTeamTool(
			{
				action: "api",
				runId: manifest.runId,
				config: { operation: "approve-plan" },
		...
		const lateCancel = await handleTeamTool(
			{
				action: "api",
				runId: manifest.runId,
		...
		assert.ok(
			["completed", "needs_attention", "blocked"].includes(loaded?.manifest.status ?? ""),
	...
	try {
		...
		const persistedTasks = [
			assessed,
			{
				...
				adaptive: {
					phase: "build",
					task: "Resume adaptive executor task",
				},
				graph: {
					taskId: "adaptive-01-executor",
					dependencies: [assessed.id],
					children: [],
...
test("adaptive workflow steps reconstruct from persisted tasks on resume", async () => {
	...
	try {
		...
		assert.ok(
			["completed", "needs_attention"].includes(result.manifest.status),
		...
		assert.ok(
			["completed", "queued", "running", "needs_attention"].includes(completed?.status ?? ""),
			`Expected executor completed/queued/running/needs_attention, got ${completed?.status}`,
		);
		assert.deepEqual(completed?.adaptive, {
			phase: "build",
			task: "Resume adaptive executor task",
		});
	} finally {
