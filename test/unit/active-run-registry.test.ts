...
import { activeRunRoots, readActiveRunRegistry, registerActiveRun, unregisterActiveRun } from "../../src/state/active-run-registry.ts";
...
test("registry ignores invalid entries", () => {
	withIsolatedHome(() => {
		const registryFile = path.join(process.env.PI_TEAMS_HOME!, ".crew", "state", "runs", "active-run-index.json");
