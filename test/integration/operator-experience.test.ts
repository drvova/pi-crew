...
test("operator mailbox compose and ack roundtrip updates delivery state", async () => {
	...
	try {
		const composed = await dispatchMailboxCompose(fixture.ctx, fixture.runId, {
			from: "operator",
			to: "worker",
			body: "hello",
			direction: "inbox",
...
test("operator mailbox ackAll acknowledges multiple unread messages", async () => {
	const fixture = createFixture();
	try {
		const first = appendMailboxMessage(fixture.manifest, {
			direction: "inbox",
			from: "a",
			to: "b",
			body: "one",
		});
		const second = appendMailboxMessage(fixture.manifest, {
			direction: "inbox",
			from: "a",
			to: "b",
			body: "two",
...
test("operator notification router respects cross-day quiet hours", () => {
	const delivered: NotificationDescriptor[] = [];
	const quiet = new NotificationRouter(
		{
			quietHours: "22:00-07:00",
			now: () => Date.parse("2026-01-01T23:30:00"),
		},
		(notification) => delivered.push(notification),
	);
	quiet.enqueue({ severity: "warning", source: "health", title: "quiet" });
	const loud = new NotificationRouter(
		{
			quietHours: "22:00-07:00",
		...
		saveRunTasks(fixture.manifest, [
			{
				...
				dependsOn: [],
				cwd: fixture.cwd,
				error: "apiToken=ZZ_LEAK_CANARY",
				heartbeat: {
					workerId: "one",
					lastSeenAt: fixture.manifest.createdAt,
					alive: true,
				},
			},
		]);
