...
test("child Pi runtime ignores observer callback failures", async () => {
	...
	try {
		const result = await runChildPi({
			...
			onJsonEvent: () => {
				throw new Error("observer write failed");
			},
			onStdoutLine: () => {
...
test("dependency output context injects prior task output and shared reads", () => {
	...
	try {
		...
		const resultArtifact = writeArtifact(manifest.artifactsRoot, {
			...
			producer: "01_explore",
			content: "Exploration output",
		});
		fs.mkdirSync(path.join(manifest.artifactsRoot, "shared"), {
			recursive: true,
		});
		fs.writeFileSync(path.join(manifest.artifactsRoot, "shared", "context.md"), "Shared context", "utf-8");
		const updatedTasks = tasks.map((task) =>
			task.stepId === "explore" ? { ...task, status: "completed" as const, resultArtifact } : task,
		);
