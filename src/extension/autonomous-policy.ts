...
const ACTIONABLE_TASK_TERMS: readonly string[] = Array.from(
	new Set([
		"implement",
		"refactor",
		"migrate",
...
function hasTaskListSignal(prompt: string): boolean {
	...
	const explicitList = ["các task", "danh sách task", "todo", "tasks sau", "task list", "làm lần lượt"].some((term) =>
		lower.includes(term),
	);
...
export function buildAutonomousPolicy(prompt: string, config: PiTeamsAutonomousConfig = {}): string {
	...
	const worktreeGuidance =
		effective.allowWorktreeSuggestion === false
	...
	return [
		...
		worktreeGuidance,
		intents.length > 0
			? `Detected pi-crew routing signals/intents in the user prompt: ${intents.join(", ")}. Consider the matching team workflow if appropriate.`
			: "No explicit pi-crew routing signal was detected; decide based on complexity, risk, task-list structure, and conflict potential.",
	].join("\n");
}
...
export function buildResourceRoutingGuidance(cwd: string, maxChars = 5000): string {
	...
	const lines = [
		...
		...(teams.length
			? teams.map(
					(team) =>
						`- ${team.name} (${team.source}): ${team.description}; defaultWorkflow=${team.defaultWorkflow ?? "default"}; roles=${team.roles.map((role) => `${role.name}->${role.agent}`).join(", ") || "none"}${team.routing?.triggers?.length ? `; triggers=${team.routing.triggers.join(",")}` : ""}${team.routing?.useWhen?.length ? `; useWhen=${team.routing.useWhen.join(";")}` : ""}`,
				)
			: ["- (none)"]),
		"Workflows:",
		...(workflows.length
			? workflows.map(
					(workflow) =>
						`- ${workflow.name} (${workflow.source}): ${workflow.description}; steps=${workflow.steps.map((step) => `${step.id}:${step.role}`).join(", ") || "none"}`,
				)
			: ["- (none)"]),
		"Agents:",
		...(agents.length
...
export function appendAutonomousPolicy(
	systemPrompt: string,
	userPrompt: string,
...
export function registerAutonomousPolicy(pi: ExtensionAPI): void {
	pi.on("before_agent_start", (event: BeforeAgentStartEvent) => {
		...
		return {
			systemPrompt: appendAutonomousPolicy(event.systemPrompt, event.prompt, autonomous, cwd),
		};
	});
}
