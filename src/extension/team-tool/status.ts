...
export function handleStatus(params: TeamToolParamsValue, ctx: TeamContext): PiTeamsToolResult {
	...
	const attentionTasks = effectiveness.needsAttentionTaskIds
		.map((id) => tasks.find((task) => task.id === id))
		.filter((task): task is (typeof tasks)[number] => task !== undefined);
	...
	const lines = [
		...
		...(manifest.runtimeResolution
			? [
					`Runtime: ${manifest.runtimeResolution.kind}`,
					`Runtime safety: ${manifest.runtimeResolution.safety}`,
					`Runtime requested: ${manifest.runtimeResolution.requestedMode}${manifest.runtimeResolution.reason ? ` (${manifest.runtimeResolution.reason})` : ""}`,
				]
			: []),
		`Goal: ${manifest.goal}`,
		...
		"Waiting tasks:",
		...(waitingTasks.length
			? waitingTasks.map((task) => `- ${task.id} [queued] ${task.role} -> ${task.agent} ${waitingReason(task, tasks) ?? "waiting"}`)
			: ["- (none)"]),
		"Completed agents:",
		...(completedAgents.length ? completedAgents.map(agentLine) : ["- (none)"]),
		"Policy decisions:",
		...(manifest.policyDecisions?.length
			? manifest.policyDecisions.map(
					(item) => `- ${item.action} (${item.reason})${item.taskId ? ` ${item.taskId}` : ""}: ${item.message}`,
				)
			: ["- (none)"]),
	...
	if (!fullDetails) {
		return result(buildCompactStatus(manifest, tasks, counts, asyncLivenessLine, phaseProgress).join("\n"), {
			action: "status",
			status: "ok",
			runId: manifest.runId,
			artifactsRoot: manifest.artifactsRoot,
			intent: `status ${manifest.runId}: ${manifest.status} (compact)`,
		});
	}
	return result(lines.join("\n"), {
		...
		runId: manifest.runId,
		artifactsRoot: manifest.artifactsRoot,
		intent: `status ${manifest.runId}: ${manifest.status}`,
	});
}
