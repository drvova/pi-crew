...
export function resolveBashCmd(): string {
	...
	const candidates = [process.env.SHELL, "C:\\Program Files\\Git\\bin\\bash.exe", "C:\\Program Files (x86)\\Git\\bin\\bash.exe"];
 ...
 */
export function resolveShellForScript(scriptPath: string): {
	command: string;
	args: string[];
} {
