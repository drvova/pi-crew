...
export function buildWidgetLines(
	...
	for (const [runIdx, { run, agents, snapshot }] of runs.entries()) {
		...
		for (const agent of visibleAgents) {
			...
			// Per-agent spinner rhythm: each agent gets its own phase offset so
			// multiple running agents don't pulse in sync (visually flat).
			...
			// ONE-LINE layout: agent name + activity + stats all on a single row.
			// Replaces the previous two-line (agent + activity sub-row) layout
			// that consumed double the vertical space for the same information.
			const allStats = [stats, activity].filter(Boolean).join(" · ");
			lines.push(truncate(`${rail}${branch} ${agentSpinner} ${name} · ${allStats}`, width));
		}
