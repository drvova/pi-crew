...
export function detectTrustPrompt(text: string): boolean {
	...
	return (
		lowered.includes("do you trust") ||
		lowered.includes("trust this") ||
		lowered.includes("untrusted") ||
		lowered.includes("workspace trust") ||
		lowered.includes("allow this folder")
	);
}
...
export function classifyStartupFailure(evidence: Omit<WorkerStartupEvidence, "classification">): StartupFailureClassification {
	...
	if (
		evidence.stderrPreview?.toLowerCase().includes("command not found") ||
		evidence.stderrPreview?.toLowerCase().includes("not recognized")
	)
...
export function createStartupEvidence(input: {
	...
	const base: Omit<WorkerStartupEvidence, "classification"> = {
		lastLifecycleState:
			input.error || (input.exitCode !== undefined && input.exitCode !== null && input.exitCode !== 0)
				? "failed"
				: input.finishedAt
					? "finished"
					: "running",
