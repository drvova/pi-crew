# Skill Verification Report

Generated: 2026-05-27T14:40:18.116Z

## Summary

| Status | Count |
|--------|-------|
| PASS | 0 |
| FAIL | 29 |
| Warnings | 0 |
| **Total** | **29** |

## Skills by Status

### Failing Skills

- **async-worker-recovery**
  - ❌ FAIL - Skill lacks enforceable gates
- **child-pi-spawning**
  - ❌ FAIL - Skill lacks enforceable gates
- **context-artifact-hygiene**
  - ❌ FAIL - Skill lacks enforceable gates
- **delegation-patterns**
  - ❌ FAIL - Skill lacks enforceable gates
- **event-log-tracing**
  - ❌ FAIL - Skill lacks enforceable gates
- **git-master**
  - ❌ FAIL - Skill lacks enforceable gates
- **live-agent-lifecycle**
  - ❌ FAIL - Skill lacks enforceable gates
- **mailbox-interactive**
  - ❌ FAIL - Skill lacks enforceable gates
- **model-routing-context**
  - ❌ FAIL - Skill lacks enforceable gates
- **multi-perspective-review**
  - ❌ FAIL - Skill lacks enforceable gates
- **observability-reliability**
  - ❌ FAIL - Skill lacks enforceable gates
- **orchestration**
  - ❌ FAIL - Skill lacks enforceable gates
- **ownership-session-security**
  - ❌ FAIL - Skill lacks enforceable gates
- **pi-extension-lifecycle**
  - ❌ FAIL - Skill lacks enforceable gates
- **post-mortem**
  - ❌ FAIL - Skill lacks enforceable gates
- **read-only-explorer**
  - ❌ FAIL - Skill lacks enforceable gates
- **requirements-to-task-packet**
  - ❌ FAIL - Skill lacks enforceable gates
- **resource-discovery-config**
  - ❌ FAIL - Skill lacks enforceable gates
- **runtime-state-reader**
  - ❌ FAIL - Skill lacks enforceable gates
- **safe-bash**
  - ❌ FAIL - Skill lacks enforceable gates
- **scrutinize**
  - ❌ FAIL - Skill lacks enforceable gates
- **secure-agent-orchestration-review**
  - ❌ FAIL - Skill lacks enforceable gates
- **state-mutation-locking**
  - ❌ FAIL - Skill lacks enforceable gates
- **systematic-debugging**
  - ❌ FAIL - Skill lacks enforceable gates
- **ui-render-performance**
  - ❌ FAIL - Skill lacks enforceable gates
- **verification-before-done**
  - ❌ FAIL - Skill lacks enforceable gates
- **widget-rendering**
  - ❌ FAIL - Skill lacks enforceable gates
- **workspace-isolation**
  - ❌ FAIL - Skill lacks enforceable gates
- **worktree-isolation**
  - ❌ FAIL - Skill lacks enforceable gates

## Recommendations

1. Fix 29 failing skills to have proper RED/GREEN gates
2. Add trigger sections (When to Activate) if missing
3. Add anti-patterns sections to prevent common mistakes
